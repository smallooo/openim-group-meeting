...
class IMUtils {
    ...
    try {
      switch (message.contentType) {
        ...
        case 110:
          ...
          Logger.print('parseMsg - 完整自定义消息数据: $map');
          
          // 首先检查是否是通话记录消息（customType: 901）
          if (map['customType'] == 901 && map['data'] != null) {
            var callData = map['data'];
            if (callData['type'] == 'audio' || callData['type'] == 'video') {
              String callType = callData['type'] == 'audio' ? '语音通话' : '视频通话';
              int duration = callData['duration'] ?? 0;
              String state = callData['state'] ?? '';
              
              if (state == 'hangup' && duration > 0) {
                // 格式化时长显示
                String formattedDuration = '通话时长 ${duration.toString().padLeft(2, '0')}:00';
                content = '$callType $formattedDuration';
              } else if (state == 'cancel') {
                content = '${callType}已取消';
              } else {
                content = callType;
              }
              Logger.print('parseMsg - 通话记录: $content');
              break;
            }
          }
  ...
  static dynamic parseCustomMessage(Message message) {
    try {
              ...
              case CustomMessageType.call:
                {
                  ...
                  Logger.print('parseCustomMessage - 通话消息解析: type=$type, state=$state, duration=$duration, content=$content');
                  if (content != null) {
                    final result = {
                    ...
                    Logger.print('parseCustomMessage - 返回通话数据: $result');
                    return result;
                  } else {
                    Logger.print('parseCustomMessage - content为空，不返回数据');
                  }
                }
                break;
      ...
      Logger.print('parseCustomMessage - 异常: $e');
      Logger.print('parseCustomMessage - 堆栈: $s');
    }
    Logger.print('parseCustomMessage - 返回null');
    return null;
  }

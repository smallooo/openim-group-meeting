...
class ReceiveRedPacketDTO {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ReceiveRedPacketDTO &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ReceiveRedPacketDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ReceiveRedPacketDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ReceiveRedPacketDTO> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ReceiveRedPacketDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ReceiveRedPacketDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ReceiveRedPacketDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
